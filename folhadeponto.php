...
if (isset($_POST['acao'])) {
    ...
    if ($_POST['acao'] === 'marcar_ponto') {
        ...
        // Calcular tipo automaticamente com base nas batidas de hoje do colaborador
        $stmt_check = $pdo->prepare("SELECT COUNT(*) as qtd FROM $tabela_registros WHERE usuario_id = :uid AND DATE(data_hora) = CURDATE()");
        $stmt_check->execute([':uid' => $usuario_id]);
        $qtd_hoje = intval($stmt_check->fetch(PDO::FETCH_ASSOC)['qtd'] ?? 0);

        if ($qtd_hoje === 0) {
            $tipo_batida = 'entrada';
        } elseif ($qtd_hoje === 1) {
            $tipo_batida = 'saida_almoco';
        } elseif ($qtd_hoje === 2) {
            $tipo_batida = 'retorno_almoco';
        } else {
            $tipo_batida = 'saida';
        }
    ...
    if ($_POST['acao'] === 'ajustar_ponto_hoje' && ($_SESSION['usuario_tipo'] ?? '') === 'master') {
        ...
        try {
            ...
            if ($target_uid === 'todos') {
                if ($is_saas) {
                    $stmt_u_list = $pdo->query("SELECT id FROM $tabela_usuarios ORDER BY nome ASC");
                } else {
...
// Calcular a próxima batida automaticamente
$tabela_registros_calc = $is_saas ? 'saas_ponto_registros' : 'ponto_registros';
$stmt_check_hoje = $pdo->prepare("SELECT COUNT(*) as qtd FROM $tabela_registros_calc WHERE usuario_id = :uid AND DATE(data_hora) = CURDATE()");
$stmt_check_hoje->execute([':uid' => $usuario_id]);
$qtd_hoje_calc = intval($stmt_check_hoje->fetch(PDO::FETCH_ASSOC)['qtd'] ?? 0);

if ($qtd_hoje_calc === 0) {
    $proxima_batida_tipo = 'entrada';
    $proxima_batida_nome = '📥 Entrada';
} elseif ($qtd_hoje_calc === 1) {
    $proxima_batida_tipo = 'saida_almoco';
    $proxima_batida_nome = '☕ Saída Almoço';
} elseif ($qtd_hoje_calc === 2) {
    $proxima_batida_tipo = 'retorno_almoco';
    $proxima_batida_nome = '🥗 Retorno Almoço';
} else {
    $proxima_batida_tipo = 'saida';
    $proxima_batida_nome = '📤 Saída';
}
...
if ($view === 'banco') {
    ...
    // 1. Obter usuários para calcular
    if (($_SESSION['usuario_tipo'] ?? '') === 'master') {
        if ($is_saas) {
            $stmt_us = $pdo->query("SELECT id, nome, email FROM $tabela_usuarios ORDER BY nome ASC");
        } else {
    ...
    <main class="ponto-main">
        ...
        ?>
            ...
            <form method="POST" action="folhadeponto.php" id="form-ponto" onsubmit="return submeterPonto(event)" style="display: flex; flex-direction: column; align-items: center;">
                ...
                <input type="hidden" name="tipo_registro" id="input-tipo-registro" value="<?= $proxima_batida_tipo ?>">

                <!-- INDICADOR DA PRÓXIMA BATIDA -->
                <div style="font-size: 15px; font-weight: 700; color: #198754; margin-bottom: 25px; text-transform: uppercase; background: rgba(25, 135, 84, 0.08); padding: 10px 20px; border-radius: 20px; border: 1px solid rgba(25, 135, 84, 0.2); display: flex; align-items: center; gap: 8px;">
                    <span>Próxima Batida:</span> <strong><?= $proxima_batida_nome ?></strong>
                </div>
            ...
            <div class="grid-ponto-full">
                ...
                <?php if (($_SESSION['usuario_tipo'] ?? '') === 'master'): ?>
                <!-- CARD DE DESTAQUE DO SALDO NO BANCO DE HORAS (APENAS MASTER) -->
                ...
                <!-- COMPROVANTES INDIVIDUAIS REP-P (DISPONÍVEIS PARA TODOS OS COLABORADORES) -->
                <div class="card-ponto" style="margin-bottom: 24px;">
                    <div class="card-header-flex">
                        <h3>🧾 Comprovantes de Ponto por Batida (Portaria 671)</h3>
                    </div>
                ...
                <div class="card-ponto">
                    <div class="card-header-flex">
                        <h3>📋 Resumo Diário da Jornada (<?= date('m/Y') ?>)</h3>
                    </div>
                            ...
                            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; width: 100%;">
                                <div class="form-control-group">
                                    ...
                                    <select name="usuario_id" required>
                                        ...
                                        if ($is_saas) {
                                            $stmt_us_sel = $pdo->query("SELECT id, nome FROM $tabela_usuarios_select ORDER BY nome ASC");
                                        } else {
            ...
            ?>
                ...
                <div class="espelho-container" style="max-width: 1000px; width: 100%; margin: 0 auto; padding: 30px; background: #fff; color: #000; border-radius: 12px; font-family: 'Courier New', Courier, monospace; box-shadow: 0 4px 15px rgba(0,0,0,0.1); border: 1px solid #ddd;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 25px; border: 1px solid #000; padding: 15px; border-radius: 6px; font-size: 13px; line-height: 1.6; background: #fafafa;">
                        ...
                        <div>
                            <?php 
                            $em_tgt = strtolower(trim($target_user['email'] ?? ''));
                            $desc_jornada = ($em_tgt === 'ti@vixmed.com.br') ? "09:30h (Seg a Qui) e 08:30h (Sex)" : "09:00h (Seg a Qui) e 08:00h (Sex)";
                            ?>
                            <strong>Carga Horária Padrão:</strong> <?= $desc_jornada ?><br>

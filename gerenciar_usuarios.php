...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao'])) {
    if ($_POST['acao'] === 'adicionar') {
        ...
        if (empty($nome) || empty($email) || empty($senha)) {
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO usuarios (nome, email, senha, funcao, setor_id, tipo) VALUES (:nome, :email, :senha, :funcao, :sid, :tipo)");
                $stmt->execute([
                    ':nome' => $nome,
                    ':email' => $email,
                    ':senha' => $senha,
...
// Buscar todos os usuários com nome do setor
$usuarios = $pdo->query("SELECT u.*, s.nome as setor_nome FROM usuarios u LEFT JOIN setores s ON u.setor_id = s.id ORDER BY u.nome")->fetchAll(PDO::FETCH_ASSOC);

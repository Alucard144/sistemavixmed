...
try {
    ...
    $stmt_chamado = $pdo->prepare("SELECT status FROM saas_chamados WHERE id = :id AND empresa_id = :empresa_id");
    $stmt_chamado->execute([':id' => $chamado_id, ':empresa_id' => $empresa_id]);
    ...
    $stmt_mov = $pdo->prepare("INSERT INTO saas_movimentacao_estoque (produto_id, tipo, quantidade, motivo, usuario_id, chamado_id) VALUES (:pid, 'saida', :qtd, :motivo, :uid, :cid)");
    $stmt_mov->execute([
        ':pid' => $produto_id,
        ':qtd' => $quantidade,
        ':motivo' => $motivo,
    ...
    $stmt_msg = $pdo->prepare("INSERT INTO saas_mensagens (chamado_id, usuario_id, mensagem) VALUES (:cid, :uid, :msg)");
    $stmt_msg->execute([
        ':cid' => $chamado_id,
        ':uid' => $_SESSION['usuario_id'],
        ':msg' => $msg_chat

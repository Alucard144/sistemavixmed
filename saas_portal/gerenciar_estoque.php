...
if (($_SESSION['usuario_tipo'] ?? '') !== 'master' || ($_SESSION['recurso_estoque'] ?? 0) != 1) { header("Location: pagina.php"); exit(); }
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao'])) {
    ...
    try {
        if ($acao === 'cadastrar_produto') {
            ...
            // Verificar duplicado
            $check = $pdo->prepare("SELECT COUNT(*) FROM saas_produtos WHERE codigo = :codigo AND empresa_id = :empresa_id");
            $check->execute([':codigo' => $codigo, ':empresa_id' => $empresa_id]);
            if ($check->fetchColumn() > 0) {
                throw new Exception("Já existe um produto cadastrado com este código!");
            }

            // Inserir produto
            $stmt = $pdo->prepare("INSERT INTO saas_produtos (empresa_id, codigo, nome, codigo_barras, categoria, quantidade, estoque_minimo) VALUES (:empresa_id, :codigo, :nome, :codigo_barras, :categoria, :quantidade, :estoque_minimo)");
            $stmt->execute([
            ...
            if ($quantidade > 0) {
                $stmt_mov = $pdo->prepare("INSERT INTO saas_movimentacao_estoque (produto_id, tipo, quantidade, motivo, usuario_id) VALUES (:pid, 'entrada', :qtd, 'Estoque inicial cadastrado', :uid)");
                $stmt_mov->execute([
                    ':pid' => $produto_id,
                    ':qtd' => $quantidade,
        ...
        elseif ($acao === 'editar_produto') {
            ...
            $check = $pdo->prepare("SELECT COUNT(*) FROM saas_produtos WHERE codigo = :codigo AND id != :id AND empresa_id = :empresa_id");
            $check->execute([':codigo' => $codigo, ':id' => $id, ':empresa_id' => $empresa_id]);
            if ($check->fetchColumn() > 0) {
                throw new Exception("Outro produto já utiliza este código!");
            }

            $stmt = $pdo->prepare("UPDATE saas_produtos SET codigo = :codigo, nome = :nome, codigo_barras = :codigo_barras, categoria = :categoria, estoque_minimo = :estoque_minimo WHERE id = :id AND empresa_id = :empresa_id");
            $stmt->execute([
                ':codigo' => $codigo,
                ':nome' => $nome,
                ':codigo_barras' => !empty($codigo_barras) ? $codigo_barras : null,
        ...
        elseif ($acao === 'movimentar_estoque') {
            ...
            $stmt_check = $pdo->prepare("SELECT nome, quantidade FROM saas_produtos WHERE id = :id AND empresa_id = :empresa_id FOR UPDATE");
            $stmt_check->execute([':id' => $produto_id, ':empresa_id' => $empresa_id]);
            ...
            // Atualizar estoque
            if ($tipo === 'entrada') {
                $stmt_up = $pdo->prepare("UPDATE saas_produtos SET quantidade = quantidade + :qtd WHERE id = :id AND empresa_id = :empresa_id");
            } else {
                $stmt_up = $pdo->prepare("UPDATE saas_produtos SET quantidade = quantidade - :qtd WHERE id = :id AND empresa_id = :empresa_id");
            }
            $stmt_up->execute([':qtd' => $quantidade, ':id' => $produto_id, ':empresa_id' => $empresa_id]);

            // Gravar log de movimentação
            $stmt_mov = $pdo->prepare("INSERT INTO saas_movimentacao_estoque (produto_id, tipo, quantidade, motivo, usuario_id) VALUES (:pid, :tipo, :qtd, :motivo, :uid)");
            $stmt_mov->execute([
                ':pid' => $produto_id,
                ':tipo' => $tipo,
                ':qtd' => $quantidade,
        ...
        elseif ($acao === 'excluir_produto') {
            ...
            $stmt = $pdo->prepare("DELETE FROM saas_produtos WHERE id = :id AND empresa_id = :empresa_id");
        ...
        elseif ($acao === 'transferir_produto') {
            ...
            $stmt_prod = $pdo->prepare("SELECT nome, codigo, quantidade FROM saas_produtos WHERE id = :id AND empresa_id = :empresa_id FOR UPDATE");
            $stmt_prod->execute([':id' => $produto_id, ':empresa_id' => $empresa_id]);
            ...
            $stmt_func = $pdo->prepare("SELECT nome FROM saas_usuarios WHERE id = :id AND empresa_id = :empresa_id");
            ...
            // Deduzir quantidade
            $stmt_dec = $pdo->prepare("UPDATE saas_produtos SET quantidade = quantidade - :qtd WHERE id = :id AND empresa_id = :empresa_id");
            $stmt_dec->execute([':qtd' => $quantidade, ':id' => $produto_id, ':empresa_id' => $empresa_id]);

            // Gravar transferência
            $stmt_trans = $pdo->prepare("INSERT INTO saas_transferencias (produto_id, funcionario_id, quantidade, observacao, usuario_id) VALUES (:pid, :fid, :qtd, :obs, :uid)");
            $stmt_trans->execute([
                ':pid' => $produto_id,
                ':fid' => $funcionario_id,
                ':qtd' => $quantidade,
            ...
            $stmt_mov = $pdo->prepare("INSERT INTO saas_movimentacao_estoque (produto_id, tipo, quantidade, motivo, usuario_id) VALUES (:pid, 'saida', :qtd, :motivo, :uid)");
            $stmt_mov->execute([
                ':pid' => $produto_id,
                ':qtd' => $quantidade,
                ':motivo' => $motivo,
        ...
        elseif ($acao === 'devolver_produto') {
            ...
            // Obter transferência
            $stmt_trans = $pdo->prepare("SELECT t.*, p.nome as produto_nome, u.nome as funcionario_nome FROM saas_transferencias t JOIN saas_produtos p ON t.produto_id = p.id JOIN saas_usuarios u ON t.funcionario_id = u.id WHERE t.id = :id AND p.empresa_id = :empresa_id FOR UPDATE");
            $stmt_trans->execute([':id' => $transferencia_id, ':empresa_id' => $empresa_id]);
            ...
            // Alterar status
            $stmt_up = $pdo->prepare("UPDATE saas_transferencias SET status = 'devolvido', data_devolucao = NOW() WHERE id = :id");
            $stmt_up->execute([':id' => $transferencia_id]);

            // Devolver para o estoque
            $stmt_add = $pdo->prepare("UPDATE saas_produtos SET quantidade = quantidade + :qtd WHERE id = :id AND empresa_id = :empresa_id");
            $stmt_add->execute([':qtd' => $trans['quantidade'], ':id' => $trans['produto_id'], ':empresa_id' => $empresa_id]);

            // Gravar log de movimentação de estoque
            $motivo = "Devolução de equipamento do funcionário: " . $trans['funcionario_nome'];
            $stmt_mov = $pdo->prepare("INSERT INTO saas_movimentacao_estoque (produto_id, tipo, quantidade, motivo, usuario_id) VALUES (:pid, 'entrada', :qtd, :motivo, :uid)");
            $stmt_mov->execute([
...
$stmt_p = $pdo->prepare("SELECT * FROM saas_produtos WHERE empresa_id = :empresa_id ORDER BY categoria, nome");
...
$sql_mov = "SELECT m.*, p.nome as produto_nome, p.codigo as produto_codigo, u.nome as usuario_nome, c.id as chamado_id 
            FROM saas_movimentacao_estoque m 
            JOIN saas_produtos p ON m.produto_id = p.id 
            JOIN saas_usuarios u ON m.usuario_id = u.id 
            LEFT JOIN saas_chamados c ON m.chamado_id = c.id
            WHERE p.empresa_id = :empresa_id
            ORDER BY m.criado_em DESC";
$stmt_mov = $pdo->prepare($sql_mov);
$stmt_mov->execute([':empresa_id' => $empresa_id]);
$movimentacoes = $stmt_mov->fetchAll(PDO::FETCH_ASSOC);

// Buscar transferências
$transferencias = [];
if (isset($_SESSION['recurso_transferencias']) && $_SESSION['recurso_transferencias'] == 1) {
    $sql_trans = "SELECT t.*, p.nome as produto_nome, p.codigo as produto_codigo, u_func.nome as funcionario_nome, u_func.email as funcionario_email, u_admin.nome as admin_nome 
                  FROM saas_transferencias t 
                  JOIN saas_produtos p ON t.produto_id = p.id 
                  JOIN saas_usuarios u_func ON t.funcionario_id = u_func.id 
                  JOIN saas_usuarios u_admin ON t.usuario_id = u_admin.id 
                  WHERE p.empresa_id = :empresa_id
                  ORDER BY t.data_transferencia DESC";
    $stmt_trans = $pdo->prepare($sql_trans);
    $stmt_trans->execute([':empresa_id' => $empresa_id]);
    $transferencias = $stmt_trans->fetchAll(PDO::FETCH_ASSOC);
}

// Buscar funcionários cadastrados para o dropdown (todos os usuários do sistema)
$funcionarios_cadastrados = [];
if (isset($_SESSION['recurso_transferencias']) && $_SESSION['recurso_transferencias'] == 1) {
    $stmt_func = $pdo->prepare("SELECT id, nome, email, funcao FROM saas_usuarios WHERE empresa_id = :empresa_id ORDER BY nome");
    $stmt_func->execute([':empresa_id' => $empresa_id]);
    $funcionarios_cadastrados = $stmt_func->fetchAll(PDO::FETCH_ASSOC);
}
    ...
    <div class="main-content">
        <div class="page-header">
            ...
            <div style="display: flex; gap: 12px;">
                <?php if (isset($_SESSION['recurso_transferencias']) && $_SESSION['recurso_transferencias'] == 1): ?>

...
// Buscar chamados
if ($tipo === 'master') {
    $sql = "SELECT c.*, u.nome as criador_nome FROM saas_chamados c JOIN saas_usuarios u ON c.usuario_id = u.id WHERE c.empresa_id = :empresa_id ORDER BY c.criado_em DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':empresa_id', $empresa_id);
} else {
    $sql = "SELECT c.*, u.nome as criador_nome FROM saas_chamados c JOIN saas_usuarios u ON c.usuario_id = u.id WHERE c.empresa_id = :empresa_id AND c.usuario_id = :uid ORDER BY c.criado_em DESC";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':empresa_id', $empresa_id);
...
$sql_sla = "SELECT criado_em, atualizado_em FROM saas_chamados WHERE empresa_id = :empresa_id AND status IN ('resolvido', 'fechado')";
if ($tipo !== 'master') {
    $sql_sla .= " AND usuario_id = :uid";
}
$stmt_sla = $pdo->prepare($sql_sla);
$stmt_sla->bindParam(':empresa_id', $empresa_id);
if ($tipo !== 'master') {
...
if ($tipo === 'master') {
    $sql_feed = "SELECT m.*, u.nome as usuario_nome, c.assunto as chamado_assunto 
                 FROM saas_mensagens m 
                 JOIN saas_usuarios u ON m.usuario_id = u.id 
                 JOIN saas_chamados c ON m.chamado_id = c.id 
                 WHERE c.empresa_id = :empresa_id
                 ORDER BY m.criado_em DESC LIMIT 5";
    $stmt_feed = $pdo->prepare($sql_feed);
    $stmt_feed->bindParam(':empresa_id', $empresa_id);
} else {
    $sql_feed = "SELECT m.*, u.nome as usuario_nome, c.assunto as chamado_assunto 
                 FROM saas_mensagens m 
                 JOIN saas_usuarios u ON m.usuario_id = u.id 
                 JOIN saas_chamados c ON m.chamado_id = c.id 
                 WHERE c.empresa_id = :empresa_id AND c.usuario_id = :uid 
                 ORDER BY m.criado_em DESC LIMIT 5";
    $stmt_feed = $pdo->prepare($sql_feed);
    $stmt_feed->bindParam(':empresa_id', $empresa_id);
